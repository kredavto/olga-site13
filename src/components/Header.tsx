  ...
  return (
    <header className="fixed inset-x-0 top-0 z-[60]">
      ...
      <motion.div
        ...
        className="overflow-hidden bg-ink text-warm-white"
      >
        <div className="mx-auto flex h-10 max-w-[1280px] items-center justify-between gap-6 px-5 lg:px-10">
      ...
      >
        <div className="mx-auto flex h-[72px] max-w-[1280px] items-center justify-between gap-6 px-5 lg:px-10 xl:gap-8">
          <a href="#hero" className="shrink-0" aria-label="На главную">
            <Wordmark tone={condensed ? "dark" : "light"} />
          </a>

          <nav className="hidden min-w-0 items-center gap-4 lg:flex xl:gap-6" aria-label="Основная навигация">
            <button
              type="button"
              className="flex items-center gap-1.5 whitespace-nowrap text-[17px] transition-opacity hover:opacity-70 xl:text-[21px]"
            ...
            {navItems.slice(1).map((item) => (
              <a
                key={item.href}
                href={item.href}
                className="whitespace-nowrap text-[17px] transition-opacity hover:opacity-70 xl:text-[21px]"
          ...
          <div className="flex shrink-0 items-center gap-3">
            <a
              ...
              className="hidden whitespace-nowrap text-[18px] font-medium tabular-nums xl:block"
            >
              {contacts.phone}
            </a>
            ...
            <button
              ...
              className={`flex size-11 items-center justify-center rounded-full border lg:hidden ${
                condensed ? "border-taupe" : "border-white/30 bg-black/25 backdrop-blur-md"
              }`}
        ...
        <AnimatePresence>
          {megaOpen && (
            <motion.div
              ...
              className="hidden border-t border-taupe/70 bg-warm-white/95 text-charcoal backdrop-blur-xl lg:block"
            >
        ...
        {mobileOpen && (
          ...
          >
            <div className="flex h-[72px] items-center justify-between px-5">
              ...
              <button
                type="button"
                className="flex size-11 items-center justify-center rounded-full border border-taupe"
            ...
            <nav className="px-5 pt-6" aria-label="Мобильная навигация">
              {navItems.map((item, index) => (
                <motion.a
                  ...
                  className="block border-b border-taupe/70 py-5 font-display text-[30px] leading-none"
                >
                  {item.label}
                </motion.a>
              ))}
