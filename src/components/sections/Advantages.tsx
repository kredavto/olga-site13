...
export default function Advantages() {
  return (
    <section className="u-grain relative overflow-hidden bg-graphite py-24 lg:py-32">
      ...
      <div className="absolute inset-0 bg-graphite/68" />
      <div className="absolute inset-0 bg-gradient-to-r from-graphite/72 via-graphite/28 to-graphite/40" />

      <div className="relative mx-auto max-w-[1400px] px-5 lg:px-10">
        <div className="grid gap-14 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] lg:gap-20">
          <Reveal>
            <h2 className="text-[clamp(2rem,3.6vw,3rem)] leading-[1.12] text-gold">
              Мы ищем причину, а не <span className="italic">маскируем</span> следствие
            </h2>
            <p className="mt-6 max-w-[44ch] text-[19px] leading-relaxed text-warm-white lg:text-[20px]">
          ...
          <ul className="grid gap-x-12 gap-y-10 sm:grid-cols-2">
            {advantages.map((item, index) => (
              <Reveal as="li" key={item.title} delay={index * 0.06}>
                ...
                <h3 className="mt-5 font-sans text-[19px] font-bold leading-snug tracking-tight text-gold">
                  {item.title}
                </h3>
                <p className="mt-2.5 text-[17px] leading-relaxed text-warm-white">{item.body}</p>
              </Reveal>
            ))}
          </ul>
        </div>
